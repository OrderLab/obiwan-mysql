...
struct Pool {
	...
	Pool(size_t size, orbit_area *obarea)
		:
		m_end(),
		m_start(),
		m_size(size),
...
struct PoolManager {
	...
	PoolManager(size_t size, orbit_area *obarea)
		:
		m_size(size),
		m_obarea(obarea)
	{
...
private:
	...
	bool add_pool(size_t n_pools)
	{
		...
		if (n_pools < m_pools.size()) {
			...
			pool = UT_NEW_NOKEY(PoolType(m_size, m_obarea));
